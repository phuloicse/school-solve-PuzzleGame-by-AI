    ...
    def validate_json(json_object: Dict[str, Any]) -> Tuple[List[str], "Constraints"]:
        ...
        for prop in ["width", "height"]:
            if key_exists(prop) and type(json_object[prop]) != int:
                errors.append("The property '{}' should be an integer but has the type '{}'.".format(prop, type(json_object[prop]) ) )

        if errors:
            return errors, None

        # check validate of each element in array json
        for prop in ["rows", "columns"]:
            if not key_exists(prop):
                continue
            # check if it is array
            if type(json_object[prop]) != list:
                errors.append("The property '{}' should be an array but has the type '{}'.".format(prop, type(json_object[prop]) ) )
                continue
            
            # check if it is valiadte int in each array in row and column
            for index, array in enumerate(json_object[prop]):
                if type(array) != list:
                    errors.append("The object at index {} of the property '{}' should be an array but actually is of type '{}'".format( index, prop, type(array) ) )
                    continue

                for number_index, number in enumerate(array):
                    if type(number) != int:
                        errors.append("The number at index {} of the array at index {} of the property '{}' should be an integer but actually has the type '{}'".format(number_index, index, prop, type(number) ) )
                        continue
                    
                    if number <= 0:
                        errors.append("The number at index {} of the array at index {} of the property '{}' is smaller or equal than zero but should actually be a positive integer.".format(number_index, index, prop) )

        if errors:
            return errors, None
        # check if number of row and column does exceeds height and width
        ...
        for index, row in enumerate(json_object["rows"]):
            if sum(row) > json_object["width"]:
                errors.append("The sum of the blocks at the row with index {} exceeds the width.".format(index) )

        for index, column in enumerate(json_object["columns"]):
            if sum(column) > json_object["height"]:
                errors.append("The sum of the blocks at the column with index {} exceeds the height.".format(index) )
