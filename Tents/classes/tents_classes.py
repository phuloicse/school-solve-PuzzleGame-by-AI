...
class BoardNode():
    ...
    def update_possible_location(self, init=False, tent_node: TentNode = None):
        ...
        if init:
            ...
            for tree in tree_dict_values:
                diagonal_to_trees = diagonal_to_trees.union(set(self.get_diagonal_adjacent_nodes(tree).values()))

                straight_adjacent_to_trees = straight_adjacent_to_trees.union(
                    set(self.get_straight_adjacent_nodes(tree).values()))

                adjacent_to_tree = adjacent_to_tree.union(set(self.get_adjacent_nodes(tree).values()))

            self.diagonal_to_trees = diagonal_to_trees
            self.straight_adjacent_to_trees = straight_adjacent_to_trees
            pure_diagonal_to_tree = diagonal_to_trees.difference(straight_adjacent_to_trees)

            pos_to_remove = pos_to_remove.union(pure_diagonal_to_tree).union(tree_set)
            ...
            for con in zeroed_contraints:
                if con.contraint_type == ConstraintNodeType.COL:
                    ...
                    pos_to_remove = pos_to_remove.union(set(self.get_whole_col(col_idx)))
                else:
                    row_idx = con.row_index
                    pos_to_remove = pos_to_remove.union(set(self.get_whole_row(row_idx)))

            possible_pos_set = set(self.possible_locations.values()).difference(pos_to_remove)
        ...
        if tent_node:
            ...
            tree_set = set(self.trees_dict.values())
            cross_adjacent_to_tent = set(self.get_straight_adjacent_nodes(tent_node).values())

            possible_tree_binded = tree_set.intersection(cross_adjacent_to_tent)
            if len(possible_tree_binded) == 1:
                ...
                adjacent_to_tree = self.get_straight_adjacent_nodes(tree_binded)
                for adjacent_node in adjacent_to_tree.values():
                    adjacents_set = set(self.get_straight_adjacent_nodes(adjacent_node).values())
            ...
            diagonal_to_tent_set = set(self.get_diagonal_adjacent_nodes(tent_node).values())
            ...
            if len(self.possible_locations) == trees_to_bind:
                ...
                for pos in self.possible_locations.values(): 
                    is_all_ok = self.place_tent_node_and_check_constraint(pos,is_shorcut_checking=True) 
                    if self.board_state == BoardStateType.INVALID: 
                        return 
                if is_all_ok == True: 
    ...
    def place_tent_node_and_check_constraint(self, tent_node, is_shorcut_checking = False): 
            ...
            if constraint_col_node.is_satisfied():
                self.remove_nodes_from_pos_list(self.get_whole_col(constraint_col_node.col_index))
            constraint_row_node.tents_number_required -= 1
            if constraint_row_node.is_satisfied():
                self.remove_nodes_from_pos_list(self.get_whole_row(constraint_row_node.row_index))
